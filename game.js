...
//     draw(offsetX) {
//         const screenX = this.x - offsetX; const currentFrame = this.currentAnimation.getCurrentFrame();
...
//         super(x, y, width, height, '#ff9f43'); this.startX = x; this.endX = endX; this.speed = speed; this.triggerX = triggerX;
...
//         else if (this.isFalling) { this.color = '#e67e22'; } else { this.color = '#ff9f43'; }
//         super.draw(offsetX);
//     }
// }
// class Projectile {
...
//     cameraX = 0; loadLevel(currentLevel);
// }
// function nextLevel() {
//     playSound('door');
//     if (currentLevel < 25) {
//         currentLevel++; document.getElementById('level').textContent = currentLevel; resetLevel();
//     } else {
//         alert(`🎉 VITÓRIA TOTAL! Você completou os 25 níveis em ${deathCount} mortes! 🏆`);
//         currentLevel = 1; deathCount = 0; document.getElementById('level').textContent = currentLevel;
//         document.getElementById('death-count').textContent = deathCount; resetLevel();
//     }
// }
...
//         }
//     } else {
//         const allSolidPlatforms = [...platforms, ...icePlatforms, ...boosterPlatforms, ...fallingFloors, ...movingFallingPlatforms];
...
function gameLoop(timestamp) {
    ...
    if (player.isDead) {
        ...
        spikes.forEach(spike => {
            if (Math.abs(player.x - spike.x) < 50 && !spike.isVisible) spike.isVisible = true;
        });
        fallingFloors.forEach(floor => {
            if (player.isOnTopOf(floor) && !floor.isFalling) floor.trigger();
        });
